...
This release introduces comprehensive Breaking Changes, paying off a large amount of accumulated technical debt and significantly reducing the cost for users getting started with Mingling for the first time.

Additionally, the `arg-picker` crate has been extracted from the mingling repository and now lives independently in its own repository: <https://github.com/catilgrass/arg-picker>. When the `picker` feature is enabled, Cargo will automatically pull it from crates.io.

> [!Warning]
>
> **MSRV**: This release raises the minimum supported Rust version (MSRV) to **1.95.0**, declared via `rust-version` in the workspace `Cargo.toml` and enforced by a new `MSRV-Check` CI job.
...
- **`structural!` (and thus `#[derive(StructuralData)]`) registers the type in all three registries** at once:
    - `RENDERERS_EXIST` — via `build_renderer_exist_entry`, so `has_renderer` routes the type to `render` (it exists even without a `#[renderer]` function).
    - `STRUCTURAL_RENDERERS` — via `build_structural_renderer_entry`, so `structural_render` serializes the type structurally.
    - `STRUCTURED_TYPES` — as before.
- **`#[renderer]` no longer emits a structural entry** — the `structural_renderer`-gated registration block in `mingling_macros/src/attr/renderer.rs` (which looked up `STRUCTURED_TYPES` and conditionally populated `STRUCTURAL_RENDERERS`) has been removed. Registering a renderer now only produces the plain-text renderer entry.
- **`#[renderer]` functions are now optional for structural formats** — a type with `#[derive(StructuralData)]` (or `structural!`) alone produces JSON / YAML / TOML / RON output via the generated `structural_render` match arm. A `#[renderer]` is only needed when a custom plain-text rendering is desired for the non-structural case.

### Migration guide

- **No code changes required for types that only used `#[derive(StructuralData)]`** — they now render structurally without a separate `#[renderer]` function.
- **Types that previously needed a dummy `#[renderer]` solely to enable structural output** can now delete that `#[renderer]` function (and any render-result pack type it referenced). The `StructuralData` derive alone produces the `has_renderer` and `structural_render` arms.
- **Types that used both `#[derive(StructuralData)]` and a real `#[renderer]` for plain-text output** keep working unchanged — the `#[renderer]` still provides the plain-text `render` arm, and the `StructuralData` derive provides the structural arms.

_Behavioral note:_ for a type with only `StructuralData` and no `#[renderer]`, calling `render` on it (the plain-text path) will now route through the generated `has_renderer` arm produced by `structural!` itself; the plain-text output is generated by the derive, so a user-supplied `#[renderer]` is only consulted when one is explicitly provided.
